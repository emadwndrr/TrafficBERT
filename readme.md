...
## `pcap` directory
Contains the code that is used to parse the pcap files and extract the flows (`pcap/flow.py`), compute and plot the dataset and flow statistics (`pcap/plot.py` and `pcap/stats.py`), and create the features from flows (`pcap/featuers.py`)
...
**important:** In `plot_feature_overlap.py`, set the `TASK` variable to either `10class` or `20class`
...
**important:** In `train.py`, set the `TASK` variable to either `10class` or `20class`

## `evaluate.py`
Contains code that loads the model form `results/<task>` and evaluates it and re-produces the confusion matrix plot.

**important:** In `evaluate.py`, set the `TASK` variable to either `10class` or `20class`
...
To evaluate run `python evaluate.py`
